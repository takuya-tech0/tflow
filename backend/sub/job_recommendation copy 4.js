...
export default function JobRecommendation() {
    ...
    const handleJobRecommendation = async () => {
        ...
        try {
            ...
            // 推奨理由をオブジェクトから取得する
            const recommendationText = response.data.recommendations; // 推奨理由テキスト
    
            // 推奨理由テキストを個々の求人に分割する
            const recommendationEntries = recommendationText.split('\n\n').filter(entry => entry.trim() !== '');
    
            recommendationEntries.forEach((entry, index) => {
                const [titleLine, ...reasonLines] = entry.split('\n').filter(line => line.trim() !== '');
                const jobTitle = titleLine.replace(/^[0-9]+\. 推奨求人：/, '').split('（求人ID')[0];
