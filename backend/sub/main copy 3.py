...
from utils import get_all_employee_data, get_all_job_posts, get_job_recommendations
...
async def recommend_jobs(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        username: str = payload.get("sub")
        if username is None:
        ...
        recommendations = get_job_recommendations(employee_data, job_posts)
