        ...
        try {
            ...
            lines.forEach((line) => {
                if (line.startsWith('1. ') || line.startsWith('2. ') || line.startsWith('3. ')) {
                    ...
                    currentJob = {
                        job_title: line.substring(3).replace('（求人ID:', '').trim(),
                    };
                    tempReasons = [];
                } else if (line.startsWith('マッチング理由') && currentJob) {
            ...
            // top_jobs から similarity を recommendations に追加
            response.data.top_jobs.forEach((job, index) => {
                if (recommendations[index]) {
                    recommendations[index].similarity = job.similarity; // 類似度を追加
                }
            });

            // "以上のように"の部分を削除
            const filteredRecommendations = recommendations.filter(rec => !rec.job_title.includes('以上のように'));
                                        ...
                                        recommendations.map((job, index) => (
                                            <li key={index} className="job-item">
                                                <div className="job-header">
                                                    <div className="job-title">{index + 1}. {job.job_title}</div> {/* 推奨求人: を削除 */}
                                                    <div className="job-similarity">{job.similarity}%</div> {/* 類似度パーセンテージの表示 */}
                                                </div>
